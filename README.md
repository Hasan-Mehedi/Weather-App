...
Weather is an Android app for Weather with Jetpack Compose. It uses [openweathermap API](https://openweathermap.org/api). This app has 3 screens. 1. splash screen 2. Information about today's weather data 3. Information about upcoming days weather summary.
...
- User opens the app. It will display the splash screen for few second  and then user sees the main weather screen with weather details of the default city - Dallas.
...
- User can click on the floating action button on the bottom to navigate to the upcoming days weather summary. It will display the weather condition, max and min temperature etc.
...
# Assumption made
1. First screen is referring to the splash screen
2. Default temperature is displayed in celsius
3. Next days weather displaying the summary instead of hourly breakdown

# Improvement to be made
1. Provide temperature display option in both celsius and fahrenheit 
2. Format the hourly temperature properly in standard hour format
3. clean up the code with moving hard coded string etc. to resource (string), const etc. class
4. add unit test coverage for all files (at least 85% code coverage)
5. improve the ui of the screen